...
//var s_account="targetmevmobileprod,targetmevglobalprod"
...
s.linkInternalFilters="javascript:,sites.target.com"
...
s._channelDomain='social media organic|facebook.com,flickr.com,twitter.com,youtube.com,myspace.com,blogspot.com,plus.google.com,reddit.com>';
s._channelPattern='email|tgt_email>internal|tgt_adv>affiliates|tgt_adv_xa>social media|tgt_soc>paid search|tgt_adv_xs>display ad|tgt_adv_xc>display ad|tgt_adv_xp>short url|tgt_adv_short>qr code|tgt_adv_qr';
s._channelParameter='display ad|dfa';
...
function s_doPlugins(s) {
	...
	s.clickPast(s.firstPage,'event27','event28');
	
	/* Store the previous page name */
	var prevPage=s.getPreviousValue(s.pageName,'gpv_pn','');
	
	/* Record the previous page name in a prop */
	s.prop17=prevPage;

	/* Call channel manager, set campaign IDs and channels */
	s.channelManager('ref','','s_cm','0','','0');
	var s_ref=s.getQueryParam('ref')
	var s_afid=s.getQueryParam('afid')
	var s_cpng=s.getQueryParam('cpng')
	var s_adgroup=s.getQueryParam('adgroup')
	var s_lnm=s.getQueryParam('lnm')
	var s_emseq=s.getQueryParam('emseq')
	var s_dfaGenesis=s.getQueryParam('dfa')
	...
	if (s._channel == 'Other Natural Referrers'){
		...
		s.campaign = 'referrer:'+ s._referringDomain[0];
		s.eVar5 = 'unpaid referrals';
	}
	...
	if(s._channel=='Natural Search'){
		...
		//set search engine
		s.eVar26='ns:'+s._partner;
		//overwrite paid search keyword
		s.eVar27='other campaigns';
		//set natural search keyword
		s.eVar28=s._keywords;
	}
	if(s._channel=='Paid Search' || s._channel=='paid search'){
		//determine whether the campaign ID is suffixed with _df
		var dfLoc = s_ref.length;
		dfLoc = dfLoc - 3;
		if(s_ref.lastIndexOf('_df') == dfLoc){
			//identify the shop comparison channel
			s._channel = s.eVar5 = 'shop comparison';
			//create campaign ID
			s.campaign=s_ref+':'+s_afid+':'+s_cpng;
			//populate afid/lnm
			s.eVar33=s_afid+":"+s_lnm;
		}
		else{
			//create campaign ID
			s.campaign=s_ref+':'+s_afid+':'+s_cpng+':'+s_adgroup;
			//set search engine
			s.eVar26='ps:'+s._partner;
			//set paid search keyword
			s.eVar27=s._keywords;
			//overwrite natural search keyword
			s.eVar28='other campaigns';
			//populate afid/lnm
			s.eVar33=s_afid+':'+s_lnm;
		}
	}
	if(s._channel=='display ad'){
		//set s.campaign for non-Genesis display ad campaigns
			s.campaign=s_ref+':'+s_afid+':'+s_cpng+':'+s_lnm;
	}
	if(s._channel=='email'){
		//create campaign ID
		s.campaign=s_ref;
	...
	if(s._channel=='affiliates'){
		//create campaign ID
		s.campaign=s_ref+":"+s_afid;
	}

	if(s._channel=='social media'){
		//create campaign ID
		s.campaign=s_ref;
	}
	if(s._channel=='social media organic'){
		//create campaign ID
		s._referringDomain = s.split(s._referringDomain,'/');
		s.campaign = 'smo:'+ s._referringDomain[0];
	}
	if(s._channel=='internal'){
		//create campaign ID
		s.campaign=s_ref;
	}
	/* Set campaign ID, landing page, pathing and stacking */
	if(!s.campaign && s.eVar5 != 'display ad')
		s.campaign=s._campaign;
	if(s.campaign)
		{
		//concatenate campaign and pageName for campaign pathing
		s.prop32=s.campaign + ':' + s.pageName;
		//set campaign stacking
		s.eVar29=s.crossVisitParticipation(s.campaign,'cvp_v29','30','5','>','purchase');
		//set campaign landing page for paid campaigns
		if(s._channel !='unpaid referrals' && s._channel !='Natural Search')
			s.eVar24=s.pageName;
		else{
			s.eVar24='natural traffic source';
		}
	}
	else{
		//set prop32 to the page in the absence of a campaign
		s.prop32=s.pageName;
	}
	if(s.campaign && !s.eVar33){
		//overwrite AFID/LNM where applicable
		s.eVar33 = 'other campaigns';
	}
	/* Search keyword pathing */
	if(s._keywords){
		//concatenate keyword and pageName
		s.prop33=s._keywords+':'+s.pageName;
	}
	else{
		//set prop33 to the page in the absence of a keyword
		s.prop33=s.pageName;
	}
	/* Overwrite keyword and partner for non-search campaigns */
	if(s.campaign && !s.eVar26)
		s.eVar26=s.eVar27=s.eVar28='other campaigns';
		
	/* Assign the campaign channel */
	if(s._channel && !s.eVar5)	
		s.eVar5=s._channel;
		
	/* Stack the campaign channels */
	s.eVar22=s.crossVisitParticipation(s.eVar5,'cvp_v22','30','5','>','purchase');
...
s.visitorNamespace="target"
s.trackingServer="target.122.2o7.net"
